...
public class verifica2010corretta {
        ...
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - 1 - i; j++) {
                if (anno[j] > anno[j + 1]) {
                    ...
                    anno[j] = anno[j + 1];
                    anno[j + 1] = temp; 
                    String tempmo = modello[j];
                    modello[j] = modello[j + 1];
                    modello[j + 1] = tempmo;
                    String tempma = marca[j];
                    marca[j] = marca[j + 1];
                    marca[j + 1] = tempma;
                    String tempde = desc[j];
                    desc[j] = desc[j + 1];
                    desc[j + 1] = tempde;
                }
            }
    ...
    // Correzioni:
    // 1. Ciclo interno modificato in (int j = 0; j < n - 1 - i; j++)
    // 2. Condizione corretta: if (anno[j] > anno[j + 1])
    // 3. Scambi eseguiti su j e j+1
    ...
    public static void stampapm(){
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - 1 - i; j++) {
                if (marca[j].compareToIgnoreCase(marca[j + 1]) > 0) {
                    int temp = anno[j];
                    anno[j] = anno[j + 1];
                    anno[j + 1] = temp; 
                    String tempmo = modello[j];
    ...
    // Correzioni:
    // 1. Ciclo interno corretto: (int j = 0; j < n - 1 - i; j++)
    // 2. Condizione corretta: if (marca[j].compareToIgnoreCase(marca[j+1]) > 0)
    // 3. Scambi tra j e j+1
    ...
    // Correzioni:
    // 1. if prima del secondo ciclo for
    // 2. secondo ciclo: (int j = i; j < n - 1; j++)

    public static void cancellam(Scanner scanner){
        scanner.nextLine();
        System.out.println("inserisci il modello da eliminare");
        String moc = scanner.nextLine();
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            System.out.println("6. stampa in ordine per marca");
